...
const HabitPacksPage = () => {
  ...
  const startPack = async (packId) => {
    console.log('[HabitPack] Start button clicked for pack:', packId);
    try {
      setLoading(true);
      await API.post(`/habit-packs/${packId}/start`);
      // Fetch the active pack after starting
      const { data: activePack } = await API.get('/habit-packs/active');
      if (activePack) {
        console.log('[HabitPack] API call successful, navigating to /daily-task with activePack:', activePack);
        navigate('/daily-task', { state: { activePack } });
      } else {
        setError('Failed to start habit pack. Please try again.');
        setLoading(false);
      }
    } catch (err) {
      console.error('[HabitPack] Failed to start habit pack:', err);
      setError('Failed to start habit pack. Please try again.');
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      <Navbar />
      <main className="flex-grow container mx-auto px-4 py-16 mt-4 flex flex-col items-center justify-center">
        <h1 className="text-3xl font-bold mb-8 text-center text-primary-text">Choose Your Habit Pack</h1>
        <div className="w-full flex justify-center">
          <div className="max-w-xl w-full">
            {packs.map((pack) => (
              <div key={pack._id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 border border-border-gray mb-6">
                <div className="p-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() => {
                        console.log('[HabitPack] Start button UI clicked for pack:', pack._id);
                        startPack(pack._id);
                      }}
