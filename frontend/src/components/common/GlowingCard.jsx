...
export const GlowingCard = ({
  ...
  return (
    <div
      className={cn(
        "relative p-6 rounded-2xl text-white w-full md:w-[340px] max-w-full",
        "bg-gray-900/80 border border-white/10 shadow-xl",
      ...
      >
        ...
        {enableGlow && (
          <div
            ...
            style={{
              WebkitMask:
                `radial-gradient(var(--glow-radius) var(--glow-radius) at var(--x, 0) var(--y, 0), var(--glow-color, #8b5cf6) 0%, transparent 70%)`,
              mask:
                `radial-gradient(var(--glow-radius) var(--glow-radius) at var(--x, 0) var(--y, 0), var(--glow-color, #8b5cf6) 0%, transparent 70%)`,
            ...
            >
              {React.Children.map(children, (child, index) => {
                if (React.isValidElement(child) && child.type === GlowingCard) {
                  const cardGlowColor = child.props.glowColor || "#8b5cf6";
